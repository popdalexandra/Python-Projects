...
def calculate_score(cards):
    ...
    if sum(cards) == 21 and len(cards) == 2: #  sau : if 11 in cards and 10 in cards and len(cards) == 2:
        return 0
...
def compare(user_score, computer_score):
    if computer_score == user_score:
        ...
        return "Lose, opponenthas Blackjack!"
    elif user_score == 0:
...
def play_game():
    ...
    for _ in range(2): # "_" nu avem nevoie de variabila ci ca loopul sa mearga de 2 ori
    ...
    while not is_game_over:
        ...
        if user_score == 0 or computer_score == 0 or user_score > 21:
            is_game_over = True
        else:       #If the game has not ended, ask the user if they want to draw another card. If yes, then use the deal_card() function to add another card to the user_cards List. If no, then the game has ended.
